...
- **週次ソースヘルスレポート**: 毎週日曜12:05 UTC（21:05 JST）に全アクティブソースの取得健全性を本人LINEにのみ通知（`services/source_health.py`）。OAuthトークンは実refreshで失効検知、データ途絶はbaseline系3日/event系45日で警告。プレビュー: `GET /api/notification/health-report`
  ...
  - **端末の「名前」と「世代」を分ける**（migration 049）。`claude_session_minutes` は `(date, host, instance_id)` で一意。`host` は人間が読む端末名で組み直しても変えず、世代はクライアントが `~/.local/state/health-ojimpo/instance_id` に永続化する uuid が持つ（端末を消せば state ごと消えて自動的に別世代になる）。**同じ host により新しい世代が現れたら旧世代は引退が確定する**ので即座に鳴り止む。以前は host 1本に両方の意味を持たせ、再構築のたびに `kagi-macbook` → `kagi-macbook-2` と改名していたため、旧 host が「引退」か「フックの故障」か区別できず120日間毎週警告が出ていた。**既存端末をv3に上げるときは、先に instance_id ファイルへ現在の host 名を書く**（書かないとその日だけ新旧2世代が同じ時間を申告して二重計上になる）
